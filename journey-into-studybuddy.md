...
Follow-up frontend refinements on **May 3, 2026** tightened the weekly checklist UX after real usage showed the course selector was too cluttered with many historical courses. Commit `a846347` (`fix: simplify study plan course navigation`) changed `PlanView` so the top setup area defaults to a compact list of selected and currently-taking courses, while `Add/remove courses` reveals the full course list only when needed. The generated checklist now uses course tabs: the user flips between courses and sees only that course's pressure points and checklist tasks. Frontend regression coverage was added for the compact selector, add/remove flow, and one-course-at-a-time tab behavior. Verification for this frontend-only refinement: frontend full suite `19 passed`; frontend production build passed with the existing large-chunk warning.

A same-day course overview fix followed after an ECB307 Canvas assignment (`courses/52441/assignments/248651`, due **May 17, 2026 at 23:59**) appeared to be missing from Cursus. The backend path already treats this as a normal assignment for a `taking` course; the actual issue was in `CoursePane`, where the home overview silently capped upcoming deadlines at six items. The fix keeps the compact default but adds a `Show all` / `Show fewer` control whenever more than six upcoming deadlines exist, so later assignments are discoverable without changing sync behavior or requiring a PAT for debugging. Regression coverage now creates seven deadlines and verifies the seventh assignment is hidden by default and visible after expanding. Verification for this frontend-only fix: focused `CoursePane` tests passed, the full frontend suite passed with `20 passed`, and the frontend production build passed with the existing large-chunk warning.

On **May 4, 2026**, real usage showed the previous diagnosis was incomplete: the ECB307 course header showed pending work while the home overview still said `Nothing pending`. Root cause investigation found two more issues. First, `CoursePane` did not render the `no_due_date` bucket, so Canvas assignments stored without a parsed due date could increment `pending_count` while disappearing from the list. Second, differentiated Canvas assignments can carry the student-facing due date in `all_dates` while top-level `due_at` is null; Cursus only requested `include[]=submission`, so it missed those dates. The sync now requests `include[]=all_dates` and falls back to the earliest visible `all_dates[].due_at` when `due_at` is null, and the overview includes undated pending work instead of showing an empty state. The same pass fixed weekly checklist noise by preferring real lecture/material files over syllabus/pages when both exist, and by skipping copyright/admin notices so generated tasks do not say to study copyright boilerplate. Verification: full backend suite `150 passed`, full frontend suite `21 passed`, and frontend production build passed with the existing large-chunk warning.

On **May 6, 2026**, the Study Plan UX was redesigned again after the course selector, tabs, and pressure point layout proved too convoluted in real use. The page now defaults to a **Weekly learning path**: one unified view with every generated course section visible, compact course rows by default, inline deadlines per course, and an `Expand {Course}` action for full detail and source chips. Course selection moved behind `Edit courses`, with search and `Update path`, so the main surface is focused on what to do this week instead of setup controls. The existing study-plan API and deterministic generator contract stayed unchanged; this was a frontend restructuring around the existing `plan.courses[]` and `plan.pressure_points[]` payload. Regression coverage now verifies that course selection is hidden until edit mode, generated courses render without tabs, collapsed courses reveal only the first two steps, expansion reveals all steps and sources, and task completion still persists. Verification during implementation: focused `PlanView` tests passed, the full frontend suite passed with `21 passed`, focused backend study-plan API tests passed with `4 passed`, `git diff --check` passed, and the frontend production build passed with the existing large-chunk warning.
